...
struct ogrenci{
	...
	char soyad[20];
	char no[5];
	int staj; // staj degeri 1'e eşitse başarılı, 0'a eşitse başarısız
	char sifre[5];
	char bolum[12];
	int bolumkod;
	int ders[20];
	...
	float final;
	int topakts;
	int mezun; //mezun degeri 1'e eşitse mezun olmus 2 ise aktif ogrenci 0'a eşitse öğrencilikten atilmis
	char dogumtarihi[20];
	char dogumyeri[20];
	char cinsiyet[6];
	int altDersSayisi;
...
int main(){
	...
	yoneticiPaneli(); //Sisteme giris icin yonetici dogrulaması yapar
...
void yoneticiPaneli(){
	...
	getch(); 
	system("CLS");//Ekrandaki yazıları temizler
	fflush(stdin);
...
void OgrenciKayidi(){
	...
	if(fPtr_ogr == NULL){
  	...
  	do{
		...
		printf("Bolum kodu      :"); //Bolumler tanıtılır ona gore bir secim yapilmasi istenir
...
void notgirisi2(){
	...
	printf("5 ve 6. yarıyıl icin not girisi.");
	char ogrencino[5];
	int i,temp,secim;
	...
	printf("Altan dersi olan öğrenci listesi:\n");
	...
	while(!feof(fPtr_alttan)){ //alttan dersi olan ogrencilerin dosyasi okunur
		...
		printf("!Dersin Ogretim Uyesinin Kimlik Dogrulamasi yapması gerekmektedir...\n");
		printf("Kullanici adi       :\n");
		gets(OgrUyeKullanici);
		fflush(stdin);
		printf("\nSifre:");
		...
		if((strcmp(OgrUyeKullanici,dersler[i].ogretmeni)==0) && (strcmp(OgrUyeSifre,dersler[i].ogrtsifre)==0) ){ //dersin ogretim uyesinin kimlik dogrulamasi
			...
			if(ort>=50){
				ogr[temp].altDersSayisi2=-1; //5. ve 6. yariyilda tamamlanan alttan derslerin sayisi azaltılır.
			}
		}
	}
	
	printf("Yeni bir ogrencinin not girisi icin 0'dan farklı bir sayi giriniz.\n");
	}while(secim!=0);
...
void stajdurumu(){
	...
	fflush(stdin);
	printf("Memur Paneline Hosgeldiniz.\nSisteme erisim icin sifre kullanici adi dogrulamasi gerekiyor.\nKullanici adi:"); //Memur girisi ile Danısman Ogretim Uyesi secilir
	gets(memurKullanici);
	fflush(stdin);
...
void OgrenciPaneli(){
	...
	if(strcmp(ogrencino,ogr[temp].no)==0 && strcmp(sifre,ogr[temp].sifre)==0){ //ogrencinin kimlik dogrulamasi
		...
		while(!feof(fPtr_notlar)){ 
			...
			if(strcmp(ogr[temp].ad,ad)==0){ //dosyadan okunan ogrenci ile girilen ogrenci aynı mı kontrol ediliyor
				printf("%s %f %f %f\n",ders,vize,final,ort); //ogrencinin dosyadan okunan not bilgileri gösteriliyor
				if(ort>50){
					printf("Basarili\n");
					ogr[temp].topakts=+5; //basarili olunan ders sayisi kadar akts arttırılıyor (1 Ders= 5 AKTS)
				}
				else{
					printf("Basarisiz\n");
				}
	...
	printf("\nOgrenci Belgesi İstiyor musunuz?(1->EVET / 2->HAYIR)");
	scanf("%d",&choice);
	if(choice==1){
...
void ogrSil(struct ogrenci s){
	...
	int bkod,durum=0;// durum 1 olduğunda kayit silinir 

	FILE *fPtr_ogr=fopen("ogrenci.txt","r");
	FILE *fPtr_yedek=fopen("yedek.txt","a");//gecici olarak ogrenci dosyası gorevi gorecek
	if( fPtr_ogr == NULL){
		puts("Dosya acilamadi");
	}
 	if( fPtr_yedek == NULL){
	...
	while(!feof(fPtr_ogr)){
		fflush(stdout);
		fscanf(fPtr_ogr,"%s %s %s %s %s %s %d\n",ad,soyad,no,sifre,yer,cins,bkod); // ogrenci dosyası okunur
    	if(s.no != no){ // Eğer ogrenci numarasi dosyada okunan numaraya eşit değilse if döngüsüne girer
   			fprintf(fPtr_yedek,"%s %s %s %s %s %s %d\n",ad,soyad,no,sifre,yer,cins,bkod); //bilgiler yedek dosyasına yazilir ve girilen ogrenci silinmis olur
    	}
    else durum=1; 
	}
	...
	if(durum == 1){
		...
		remove("yedek.txt"); // yedek dosyası kullanılmadıgı ıcın silinmis
	}
